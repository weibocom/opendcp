    ...
    if($action){
      if($strList = $thisClass->get($myUser, $this->module, $action, $param , $id)){
        ...
        if(isset($arrList['code']) && $arrList['code'] == 0){
          $retId=(isset($arrList['data']))?(int)$arrList['data']:0;
          $ret = array(
            'code' => 0,
            'msg' => 'success',
            'retId' => $retId,
  ...
  function getList($myUser = '', $param = array()){
    ...
    if($strList = $thisClass->get($myUser, $this->module, 'list', $param)){
      ...
      if(isset($arrList['code']) && $arrList['code'] == 0 && isset($arrList['data'])){
        ...
        $ret['count'] = (isset($arrList['count'])) ? $arrList['count'] : 0;
        $ret['pageCount'] = (isset($arrList['total_page'])) ? $arrList['total_page'] : 1;
        $ret['page'] = (isset($arrList['page'])) ? $arrList['page'] : 1;
        $i=0;
        foreach($arrList['data'] as $k => $v){
          $i++;
  ...
  switch($myAction){
    ...
    case 'insert':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        ...
        if(isset($arrJson['params'])&&empty($arrJson['params'])) $arrJson['params']=(object)$arrJson['params'];
        /* 命令发现Begin */
        $tType=$arrJson['type'];
        ...
        /* 命令发现Begin */
        if(isset($retArr['retId'])&&!empty($retArr['retId'])){
          $arrJson1=array(
            'action_id' => $retArr['retId'],
            'type' => $tType,
            'template' => $tTemplate,
            'task_type' => $taskType,
          );
          $retArr1 = $myDepend->update($myUser,'create', $arrJson1);;
        }
      }
      break;
    case 'update':
...
if($logFlag){
  ...
  if(isset($arrJson1)) $arrRecodeLog['t_code'] .= '外部接口传入：' . json_encode($arrJson1,JSON_UNESCAPED_UNICODE) . "\n\n";
  if(isset($retArr1)) $arrRecodeLog['t_code'] .= '外部接口返回：' . str_replace(array("\n", "\r"), '', $retArr1['ret']) . "\n\n";

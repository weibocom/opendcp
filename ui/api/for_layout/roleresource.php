...
class myself{
    private $module = 'roleresource';
    function getList($myUser = '', $param = array(),$myType =''){
        global $thisClass;
        $ret=array('code' => 1, 'msg' => 'Illegal Request', 'ret' => '');
        if($myType){
            $list_action='list/'.$myType;
        }else{
            $list_action='list';
        }
        if($strList = $thisClass->get($myUser, $this->module, $list_action, $param)){
...
$myType = (isset($_POST['type'])&&!empty($_POST['type']))?trim($_POST['type']):((isset($_GET['type'])&&!empty($_GET['type']))?trim($_GET['type']):'');
...
if($hasLimit){
    ...
    switch($myAction){
        case 'list':
            ...
            $retArr = $mySelf->getList($myUser, $arrJson,$myType);
            $retArr['page'] = $myPage;
        ...
        case 'insert':
            ...
            if(isset($arrJson) && !empty($arrJson)){
                ...
                //if(!empty($arrJson['param_values'])) $arrJson['param_values']=json_encode($arrJson['param_values']);
        ...
        case 'update':
            ...
            if(isset($arrJson) && !empty($arrJson)){
                ...
                $role_id=$arrJson['id'];
                unset($arrJson['id']);
                $retArr=$mySelf->update($myUser, 'update', $arrJson,$role_id );
                $logDesc = (isset($retArr['code']) && $retArr['code'] == 0) ? 'SUCCESS' : 'FAILED';
            }
            break;
        case 'delete':

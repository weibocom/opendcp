...
class myself{
  ...
  function getList($myUser = '', $param = array()){
    ...
    if($strList = $thisClass->get($myUser, 'cluster/'.$param['cluster_id'], 'list_services', $param)){
...
$fClusterId=(isset($_POST['fClusterId'])&&!empty($_POST['fClusterId']))?trim($_POST['fClusterId']):((isset($_GET['fClusterId'])&&!empty($_GET['fClusterId']))?trim($_GET['fClusterId']):'');
  ...
  switch($myAction){
    ...
    case 'insert':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        ...
        if(isset($arrJson['cluster_id'])) $arrJson['cluster_id']=(int)$arrJson['cluster_id'];

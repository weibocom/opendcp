  ...
  switch($myAction){
    ...
    case 'insert':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        ...
        $stepsObject=array();
        foreach($arrJson['steps'] as $v){
          if(isset($v['param_values'])&&empty($v['param_values'])) $v['param_values']=(object)array();
          if(isset($v['retry'])&&empty($v['retry'])) $v['retry']=(object)array();
          $stepsObject[]=$v;
        }
        $arrJson['steps']=$stepsObject;

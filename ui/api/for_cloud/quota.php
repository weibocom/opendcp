...
class myself{
  ...
  function getInfo($myUser = '', $idx = ''){
    ...
    if($strList = $thisClass->get($myUser, $this->module.'/bill', 'GET', '', $idx)){
  ...
  function update($myUser = '', $method = 'POST', $param = array()){
    global $thisClass;
    $ret = array('code' => 1, 'msg' => 'Illegal Request', 'ret' => '');
    if($param){
      $idx='';
      ...
      if($strList = $thisClass->get($myUser, $this->module.'/credit', $method, '', $idx)){
        $arrList = json_decode($strList,true);
...
$fHour=(isset($_POST['fHour'])&&!empty($_POST['fHour']))?trim($_POST['fHour']):((isset($_GET['fHour'])&&!empty($_GET['fHour']))?trim($_GET['fHour']):'');
...
if($hasLimit){
  ...
  switch($myAction){
    ...
    case 'update':
      ...
      if(!empty($arrJson)){
        $fIdx=(isset($arrJson['id']))?(int)$arrJson['id']:0;
        $fHour=(isset($arrJson['hours']))?(int)$arrJson['hours']:0;
        $arrJson=array($fIdx,$fHour);

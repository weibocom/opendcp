...
class myself{
  private $module = 'slb';
  private $sub_module = 'listener';

  function getInfo($myUser = '', $param = array()){
    global $thisClass;
    $ret=array('code' => 1, 'msg' => 'Illegal Request', 'ret' => '');
    if($strList = $thisClass->get($myUser, $this->module.'/'.$this->sub_module, 'GET', $param)){
  ...
  function active($myUser = '', $action = '', $method = 'POST', $param = array(), $id = ''){
    global $thisClass;
    $ret = array('code' => 1, 'msg' => 'Illegal Request', 'ret' => '');
    if($param){
      if($strList = $thisClass->get($myUser, $this->module.'/'.$this->sub_module.'/'.$action, $method, $param , $id)){
        $arrList = json_decode($strList,true);
        if(isset($arrList['code']) && $arrList['code'] == 0){
          $ret = array(
            'code' => 0,
...
$fProtocol=(isset($_POST['fProtocol'])&&!empty($_POST['fProtocol']))?trim($_POST['fProtocol']):((isset($_GET['fProtocol'])&&!empty($_GET['fProtocol']))?trim($_GET['fProtocol']):'');
$fPort=(isset($_POST['fPort'])&&!empty($_POST['fPort']))?trim($_POST['fPort']):((isset($_GET['fPort'])&&!empty($_GET['fPort']))?trim($_GET['fPort']):'');
...
if($hasLimit){
  ...
  switch($myAction){
    ...
    case 'update':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        $retArr = $mySelf->update($myUser,'PUT', $arrJson);
        $logDesc = (isset($retArr['code']) && $retArr['code'] == 0) ? 'SUCCESS' : 'FAILED';
      }
      break;
    case 'delete':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        $retArr=$mySelf->update($myUser, 'DELETE', $arrJson);
        $logDesc = (isset($retArr['code']) && $retArr['code'] == 0) ? 'SUCCESS' : 'FAILED';
      }
      break;
    case 'start':
      ...
      if(!empty($arrJson)){
        $retArr = $mySelf->active($myUser, $myAction, 'POST', $arrJson);
        $logDesc = (isset($retArr['code']) && $retArr['code'] == 0) ? 'SUCCESS' : 'FAILED';
      }
      break;
    case 'stop':
      if($myStatus > 0){ $retArr['msg'] = 'Permission Denied!'; break; }
      $arrRecodeLog['t_action'] = '停止';
      if(!empty($arrJson)){
        $retArr = $mySelf->active($myUser, $myAction, 'POST', $arrJson);
        $logDesc = (isset($retArr['code']) && $retArr['code'] == 0) ? 'SUCCESS' : 'FAILED';
      }

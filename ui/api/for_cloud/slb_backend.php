...
class myself{
  private $module = 'slb';
  private $sub_module = 'backendservers';

  function getList($myUser = '', $type='healthstatus', $param = array(), $id=''){
    global $thisClass;
  ...
  switch($myAction){
    ...
    case 'insert':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        $retArr = $mySelf->update($myUser,'POST', $arrJson, $fIdx);
    ...
    case 'update':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        $retArr = $mySelf->update($myUser,'PUT', $arrJson, $fIdx);
        $logDesc = (isset($retArr['code']) && $retArr['code'] == 0) ? 'SUCCESS' : 'FAILED';
      }
      break;
    case 'delete':
      ...
      if(!empty($fIdx) && !empty($arrJson)){
        $retArr=$mySelf->update($myUser, 'DELETE', $arrJson, $fIdx);
        $logDesc = (isset($retArr['code']) && $retArr['code'] == 0) ? 'SUCCESS' : 'FAILED';
      }
      break;
  }

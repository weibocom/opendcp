...
class myself{
  ...
  private $subModule = 'main';
  private $arrFormat = array();

  function getList($param = array()){
    global $thisClass;
    $ret=array('code' => 1, 'msg' => 'Illegal Request', 'ret' => '');
    if($strList = $thisClass->get($this->module, $this->subModule, 'list_ver', $param)){
  ...
  function getVerList($param = array()){
    global $thisClass;
    $ret=array('code' => 1, 'msg' => 'Illegal Request', 'ret' => '');
...
if($hasLimit){
  ...
  switch($myAction){
    ...
    case 'ver_list':
      ...
      if(!$fIdx) unset($arrJson['name']);
      $retArr = $mySelf->getVerList($arrJson);
      if(count($retArr['content'])>$myPageSize) $myPageSize=count($retArr['content']);
      $retArr['page'] = $myPage;
      $retArr['pageSize'] = $myPageSize;
      if(!isset($retArr['pageCount'])||$retArr['pageCount']<1) $retArr['pageCount']=1;

...
class myself{
  ...
  private $subModule = 'appkey';
  private $arrFormat = array();

  function getList($param = array()){
    global $thisClass;
    $ret=array('code' => 1, 'msg' => 'Illegal Request', 'ret' => '');
    if($strList = $thisClass->get($this->module, $this->subModule, 'list_privilege', $param)){
...
if($hasLimit){
  ...
  switch($myAction){
    case 'list':
      ...
      if(!$fIdx) unset($arrJson['addr']);
      $retArr = $mySelf->getList($arrJson);
      $myPageSize=count($retArr['content']);
      $retArr['page'] = $myPage;
    ...
    case 'insert':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        ...
        $arrJson['user'] = $myUser;
        $retArr = $mySelf->update('add_privilege', $arrJson);
    ...
    case 'delete':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        $arrJson['user'] = $myUser;
        $retArr=$mySelf->update('delete_privilege', $arrJson);
        $logDesc = (isset($retArr['code']) && $retArr['code'] == 0) ? 'SUCCESS' : 'FAILED';
      }
    break;
  }

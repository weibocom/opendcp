...
class myself{
  ...
  private $subModule = 'auto_alteration';
  ...
  function getInfo($action='type_param',$param = array()){
    global $thisClass;
    $ret=array('code' => 1, 'msg' => 'Illegal Request', 'ret' => '');
...
$fType=(isset($_POST['fType'])&&!empty($_POST['fType']))?trim($_POST['fType']):((isset($_GET['fType'])&&!empty($_GET['fType']))?trim($_GET['fType']):'');
...
if($hasLimit){
  ...
  switch($myAction){
    ...
    case 'info':
      ...
      $retArr = $mySelf->getInfo('type_param',$arrJson);
      break;
    case 'state':
      $logFlag = false;//本操作不记录日志
      $arrJson = array(
      ...
      $retArr = $mySelf->getInfo('check_state',$arrJson);
      break;
    case 'insert':
    ...
    case 'delete':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        $arrJson['user'] = $myUser;
        $retArr=$mySelf->update('remove', $arrJson);
        $logDesc = (isset($retArr['code']) && $retArr['code'] == 0) ? 'SUCCESS' : 'FAILED';
      }
    break;
  }

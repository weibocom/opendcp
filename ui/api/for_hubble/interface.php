...
class myself{
  ...
  function getList($param = array()){
    ...
    if($strList = $thisClass->get($this->module, $this->subModule, 'list_interface', $param)){
  ...
  switch($myAction){
    ...
    case 'insert':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        ...
        $arrJson['user'] = $myUser;
        $retArr = $mySelf->update('add_interface', $arrJson);
    ...
    case 'delete':
      ...
      if(isset($arrJson) && !empty($arrJson)){
        $arrJson['user'] = $myUser;
        $retArr=$mySelf->update('delete_interface', $arrJson);
        $logDesc = (isset($retArr['code']) && $retArr['code'] == 0) ? 'SUCCESS' : 'FAILED';
      }
    break;
  }

  ...
  if(data.content){
    if(data.content.length>0){
      ...
      for (var i = 0; i < data.content.length; i++) {
        ...
        td = '<td>' + v.i + '</td>';
        tr.append(td);
        td = '<td><a class="tooltips" title="查看脚本详情" data-toggle="modal" data-target="#myViewModal" onclick="view(\'shell\',\''+v.id+'\')">' + v.name + '</a></td>';
        tr.append(td);
        td = '<td>' + v.desc + '</td>';
        tr.append(td);
        td = '<td>' + v.opr_user + '</td>';
        ...
        btnDel = '<a class="text-danger tooltips" title="删除" data-toggle="modal" data-target="#myModal" onclick="twiceCheck(\'del\',\''+v.id+'\',\''+v.name+'\',\''+v.desc+'\')"><i class="fa fa-trash-o"></i></a>';
          ...
          if(typeof(data.content)!='undefined'){
            ...
            switch(type){
              case 'shell':
                var str='';
                var locale={};
                if(typeof(locale_messages.hubble.shell)) locale = locale_messages.hubble.shell;
                $.each(data.content,function(k,v){
                  if(k=='content'){
                    str+='<div class="row col-sm-12"><hr class="col-sm-12" style="margin-top: 5px;margin-bottom: 5px;"></div>' +
                      '<h5 class="col-sm-12 text-primary"><strong>脚本内容</strong></h5>' +
...
//二次确认
var twiceCheck=function(action,idx,desc,pidx){
  NProgress.start();
  if(!idx) idx='';
  if(!desc) desc='';

...
var showRole=function(){
    if($('#cmd_child').val()=='role'){
        $('#name').attr("placeholder","仅支持大小写字母数字和下划线 eg:test_1");
                ...
                if(data.code==0){
                    ...
                    for(var i=0;i<data.content.length;i++){
                        ...
                        switch (v.resource_type){
                            case 'var':
                                var var_checkboxes = '<label style="margin:10px"><input type="radio" id="'+v.id+'" name="var">'+v.name+'</label>';
                                $('#vars_file').append(var_checkboxes);
                                break;
                            case "template":
                                var tem_checkboxes = '<label style="margin:10px"><input type="checkbox" id="'+v.id+'" name="template">'+v.name+'</label>';
                                $('#tems_file').append(tem_checkboxes);
                                break;
                            case 'task':
                                var task_checkboxes = '<label style="margin:10px"><input type="radio" id="'+v.id+'" name="task">'+v.name+'</label>';
        ...
        $('#name').attr("placeholder","名称,eg:test");
        $("#cmd_content").val('');

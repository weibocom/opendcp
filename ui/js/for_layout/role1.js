...
var list = function(page,tab) {
    ...
    switch (tab) {
        case 'resource':
            ...
            postData['resource_type']=$('#resourceType').val();
            break;
        case 'role':
    ...
    if(data.content){
        if(data.content.length>0){
            ...
            for (var i = 0; i < data.content.length; i++) {
                ...
                switch(tab){
                    case 'resource':
                        td = '<td><a class="tooltips" title="查看详情" data-toggle="modal" data-target="#myViewModal" onclick="view(\'resource\',\''+v.id+'\')">' + v.name + '</a></td>';
                        tr.append(td);
                        td = '<td>' + v.type + '</td>';
                        tr.append(td);
                        td = '<td>' + v.lastOp + '</td>';
                        tr.append(td);
                        td = '<td>' + getDate(v.UpdatedTime) + '</td>';
                        tr.append(td)
            ...
            success: function (data) {
                ...
                if(data.code==0){
                    if(typeof(data.content)!='undefined'){
                        ...
                        $('#task_file').html('');
                        // alert("hhh");
                        // alert(data.content.vars);
                        if(data.content.vars!=" "){
                            var var_array = data.content.vars.split(",");
                            for(var i=0;i<var_array.length;i++){
                                var var_checkboxes = '<input type="checkbox" class="form-control"  id="'+var_array.id+'name="var">'+var_array.id;
                        ...
                        if(data.content.tasks!=" "){
                            ...
                            for(var i=0;i<task_array.length;i++){
                                var task_checkboxes = '<input type="checkbox" class="form-control"  id="'+var_array.id+'name="var">'+var_array.id;
                                $('#task_file').append(task_checkboxes);
                            }
                        }
                        if(data.content.templates!=" "){
                            var tem_array = data.content.tasks.split(",");
                            for(var i=0;i<tem_array.length;i++){
                                var tem_checkboxes = '<input type="checkbox" class="form-control"  id="'+var_array.id+'name="task">'+var_array.id;

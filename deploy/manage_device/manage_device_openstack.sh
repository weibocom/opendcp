...
echo 'INSECURE_REGISTRY="--insecure-registry docker.io"' >> /etc/sysconfig/docker
...
#47.93.162.228
docker run -d -e "mysql_url=$1" -e "get_key_url=$2" -e "report_url=$3" -e "instance_id=$4" -e "ssh_port=$7" --net=host --name octans-agent registry.cn-beijing.aliyuncs.com/opendcp/octans-agent:latest
